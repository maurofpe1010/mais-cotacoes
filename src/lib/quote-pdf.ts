...
export function buildQuotePdf(input: PdfInput): string {
  ...
  const networkPlans = groups.flatMap(([name, columns]) => {
    const plans = new Map<string, { plan: PdfPlan; accommodations: string[] }>();
    for (const { plan, accommodation } of columns) {
      const entry = plans.get(plan.id) ?? { plan, accommodations: [] };
      if (!entry.accommodations.includes(accommodation)) entry.accommodations.push(accommodation);
      plans.set(plan.id, entry);
    }
    return [...plans.values()].map(entry => ({ name, ...entry }));
  });
  for (let offset = 0; offset < networkPlans.length; offset += 4) {
    const cards = networkPlans.slice(offset, offset + 4).map(({ name, plan, accommodations }) => {
