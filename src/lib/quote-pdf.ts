...
export function buildQuotePdf(input: PdfInput): string {
  ...
  for (let offset = 0; offset < networkPlans.length; offset += 6) {
    const cards = networkPlans.slice(offset, offset + 6).map(({ name, plan, accommodations }) => {
