...
export default function QuotesPage() {
  ...
  async function loadData() {
    ...
    const normalizedLeads = ((leadRows ?? []) as unknown as CrmLeadQueryRow[]).map((lead) => ({ ...lead, primary_contact: firstRelation(lead.primary_contact) ?? null }));
    const normalizedQuotes = ((quoteRows ?? []) as unknown as QuoteQueryRow[]).map((quote) => ({ ...quote, lead: firstRelation(quote.lead) ?? null }));
  ...
  async function deleteQuote(quote: Quote) {
    if (!window.confirm(`Excluir a cotação “${cleanQuoteTitle(quote.title)}”? Ela ficará arquivada e poderá ser recuperada futuramente.`)) return;
