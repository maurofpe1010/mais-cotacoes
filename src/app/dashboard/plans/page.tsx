...
export default function PlansPage() {
  ...
  function reset() { setSection(null); setEditingInsurer(null); setEditingPlan(null); setLegalName(""); setTradeName(""); setRegistration(""); setInsurerId(""); setProductName(""); setPlanName(""); setPlanCode(""); setAccommodation("apartment"); setCopay(""); setLogoFile(null); setError(""); }
  ...
  function editPlan(item: Plan) { reset(); setSection("plan"); setEditingPlan(item); setInsurerId(item.insurer_id); setProductName(item.product?.name ?? ""); setPlanName(item.name); setPlanCode(item.plan_code ?? ""); setAccommodation(item.accommodation); setCopay(item.copay_description ?? ""); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  async function savePlan(event: FormEvent) {
    ...
    try {
      ...
      const values = { name: planName, plan_code: planCode || null, accommodation, copay_description: copay || null, ...(logoUrl ? { logo_url: logoUrl } : {}) };
