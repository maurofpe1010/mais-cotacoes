...
export default function DashboardLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <div className="shell"><aside className="sidebar"><div><OrganizationLogo /><p style={{ fontSize: 12, color: "#b6d2f7" }}>Gestão comercial de benefícios</p></div><nav className="nav">{links.map(([label, href]) => <Link key={href} href={href} className={href === "/dashboard" ? "active" : ""}>{label}</Link>)}</nav><div className="account">Sua corretora<br /><Link href="/dashboard/settings" style={{ color: "white", fontWeight: 700 }}>Configurações</Link></div></aside><div className="main"><div className="mobile-app-header"><img src="/mais-corretora-logo.png" alt="Mais Corretora" className="mobile-brand-logo" /><nav className="mobile-nav">{links.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</nav></div><BackButton />{children}<CatalogImportShortcuts /><PlanHospitalsManager /><PricingEditShortcuts /></div></div>;
}
