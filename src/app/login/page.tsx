...
export default function LoginPage() {
  const router = useRouter(); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [message, setMessage] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(false); const [splash, setSplash] = useState(true); const [logoUrl, setLogoUrl] = useState<string | null>(null);
  useEffect(() => { setLogoUrl(window.localStorage.getItem("mais-cotacoes-logo")); const timer = window.setTimeout(() => setSplash(false), 1800); return () => window.clearTimeout(timer); }, []);
  ...
  const logo = logoUrl ? <img src={logoUrl} alt="Logo da corretora" className="access-logo-image" /> : <div className="access-logo-text">MAIS <span>CORRETORA</span></div>;
